...
- They are re-calculated even when they are not rendered in a component or hook. This ensures they have the most up-to-date calculated value before they are even rendered. This also makes them the perfect solution for background syncing, because they can get other atom's values without subscribing to their updates by using the `getValue` function
